...
Routes.prototype.dashboard = function(req, res) {
  ...
  if(username && password) {
    ...
    User.findOne({ username: username, password: password }, function(err, user) {
      if(user) {
        // user is logged in
        res.render('dashboard', { email: user.email, username: user.username });
      } else {
...
Routes.prototype.login = function(req, res) {
  ...
  User.findOne({ username: username, password: password }, function(err, user) {
    if(user) {
      ...
      response = { success: true, access_token: access_token, email: user.email, username: user.username };
  ...
  if(username && password) {
    ...
    User.findOne({ username: username, password: password }, function(err, user) {
      if(user) {
        ...
        res.render('remote_login', { layout: false, url: url, access_token: access_token, email: user.email, username: user.username });
      } else {
        // user is not logged in
