...
Graphedia.prototype.setup_socket = function() {
  ...
  _g.socket.on('comments.new', function(data) {
    _g.container.append(
      new Comment('add', { comment_id: data.comment_id, comment: data.comment, author: data.author, comment_stream_id: data.comment_stream_id, x: data.x, y: data.y, ups: 0 })
    );
  })
...
Graphedia.prototype.add_markup = function() {
  ...
  $g(window).click(function(e) {
    if(e.shiftKey && _g.user.access_token) {
      var new_comment = new Comment('create', { author: _g.user, comment_stream_id: 0, x: e.pageX, y: e.pageY })
...
function Comment(method, params) {
  ...
  this.comment_stream_id = params.comment_stream_id || 0;
  this.x = params.x || 0;
  this.y = params.y || 0;
...
Comment.prototype.add = function() {
  ...
  var comment = $g('<div>').addClass('comment').css({
    ...
    'top': c.y+'px',
    'left': c.x+'px'
  }).html(c.author + ' says ' + c.comment)
    .attr('id',c.comment_id)
    .click(function() {
    c.upvote();
  })
  
  var reply = $g('<a>').html('reply').click(function() {
    var new_comment = new Comment('create', { author: _g.user, comment_stream_id: c.comment_stream_id, x: c.x, y: c.y })
    _g.container.append(new_comment.markup);
    new_comment.obj.ready(_g, new_comment.markup, new_comment.comment, new_comment.form);
  })
  ...
  comment.append($('<br />'),reply,$('<br />'),ups)
...
Comment.prototype.create = function() {
  ...
  var new_comment = $g('<div>').addClass('new-comment').css({ top: (c.y-54), left: (c.x-1) }); //i don't know why it starts out 50 pixels below, but whatever.
  var form = $g('<form>',{name:'new_comment'});
  var comment = $g('<textarea>').attr('class','commentField').attr('name','new_comment');
...
Comment.prototype.ready = function(g, markup, comment, form) {
  ...
  form.submit(function() {
    ...
    g.socket.emit('comments.new', { access_token: _g.user.access_token, comment: body, comment_stream_id: c.comment_stream_id, x: c.x, y: c.y }, function(data) {
      if(data.success) {
        markup.remove();
        g.container.append(
          new Comment('add', { comment_id: data.comment_id, comment: body, author: _g.user, comment_stream_id: data.comment_stream_id, x: c.x, y: c.y, ups: 0 })
        );
      }
    })
    return false;
