...
GC.login = function(data) {
  if(data.success) {
    _md.markup.empty();
    _g.mini_dash.returning(data.email, data.username);
  } else { _md.markup.find('.username, .password').addClass('form-error') }
}
...
MiniDash.prototype.guest = function() {
  // add register link
  var register = $g('<div>').addClass('register');
  var register_a = $g('<a>',{href:'http://www.graffidia.com',target:'_blank'}).html('Register');
  register.append(register_a);
  
  // add login form
  var login = $g('<div>').addClass('login');
  
  var login_form = $g('<form>',{name:'login',id:'login_form'});
  
  var login_username = $g('<input>',{type:'text',name:'username',value:'username',title:'username'}).addClass('username italic');
  login_username.focus(function() { if($g(this).val() == $g(this).attr('title')) $g(this).val('').removeClass('italic'); })
  login_username.blur(function() { if($g(this).val() == '') $g(this).val($g(this).attr('title')).addClass('italic') })
  
  var login_password = $g('<input>',{type:'password',name:'password'}).addClass('password');
  
  login_form.append(login_username,login_password);
  
  login.append(login_form);
  ...
  login_form.submit(function() {
    ajax.login(login_username.val(), login_password.val())
    return false;
  })
  
  // add clear fix
  var clear = $g('<div>',{style:'clear:both;'});
  
  _md.markup.append(register,login,clear);
  
  _md.markup.slideDown('fast')
}
MiniDash.prototype.returning = function(email, username) {
  _md.markup.append("Hey " + username + ", what's good?");
  _md.markup.slideDown('fast', function() {
    setTimeout(function() {
