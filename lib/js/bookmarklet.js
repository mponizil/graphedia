...
Comment.prototype.create = function() {
  ...
  var new_comment = $g('<div>').addClass('new-comment').css({top:c.y,left:c.x});
  var form = $g('<form>',{name:'new_comment'});
  var comment = $g('<input>').attr('type','text').attr('name','new_comment');
  var submit = $g('<input>').attr('type','submit').attr('name','submit').attr('value','Post');
