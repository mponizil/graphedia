...
Graphedia.prototype.add_markup = function() {
  ...
  $g(window).click(function(e) {
    if(e.shiftKey && _g.user.access_token) {
      ...
      _g.comment_manager.add(new_comment);
      new_comment.ready();
    }
  })
}
...
Graphedia.prototype.display_comments = function(comments) {
  for(i in comments) {
    _g.display_replies(comments[i])
  }
}
Graphedia.prototype.display_replies = function(comment) {
  var reply = new Comment('add', {
...
CommentManager.prototype.add = function(comment) {
...
Comment.prototype.add = function() {
  ...
  c.markup = $g('<div>').addClass('commentContainer').attr('id',c.comment_id).click(function() {
...
Comment.prototype.create = function() {
  ...
  var form = $g('<form>',{name:'new_comment'}).attr('id','newCommentFormId');
...
Comment.prototype.ready = function() {
  ...
  var markup = c.markup;
  var form = c.markup.find('#newCommentFormId')
  var comment = c.markup.find('#newCommentId')
  ...
  form.submit(function() {
    ...
    _g.socket.emit('comments.new', {
      ...
      if(data.success) {
        var new_comment = new Comment('add', {
          comment_id: data.comment_id,
          comment: body,
          author_id: _g.user._id,
          author: _g.user.username,
...
Comment.prototype.magic = function(isActive) {
  ...
  if(isActive){
    ...
    $g('#' + _activeComment).animate({
      ...
      if (c.comment_id == _activeComment) {
        _activeComment = null;
        return;
      }
  }
  
  _activeComment = c.comment_id;
  _activeCoords = [c.x, c.y]
