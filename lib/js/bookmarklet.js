...
Graphedia.prototype.add_markup = function() {
  ...
  $g(window).click(function(e) {
    if(e.shiftKey && _g.user.access_token) {
      ...
      new_comment.obj.ready(new_comment.markup, new_comment.comment, new_comment.form);
    }
  })
}
...
Graphedia.prototype.display_replies = function(comment) {
  var ul = $g('<ul>');
  var li = $g('<li>');
  var div = $g('<div>').addClass('comment-block').attr('id','cp_' + comment._id)
  ul.append(li);
  li.append(div)
  
  var comment_markup = new Comment('add', {
...
function Comment(method, params) {
  ...
  if(method == 'add') return this.add(params.parent_div);
  else if(method == 'create') return this.create();
}

Comment.prototype.add = function() {
  var c = this;
  var container = $g('<div>').addClass('commentContainer').attr('id',c.comment_id).click(function() {
    c.upvote();
...
Comment.prototype.ready = function(markup, comment, form) {
  var c = this;
