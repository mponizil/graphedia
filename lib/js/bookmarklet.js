...
function lab_js_loaded() {
  ...
  .script("http://" + URL + "/javascripts/flyOffPage.js")
...
function Graphedia() {
  ...
  // $g(window).bind('keydown', function(e){ //live?
  //   if (e.keyCode == 192){
  //     if ($g("#graphedia_container").children().length != 0) {
  //       console.log($g('#graphedia_container ul'))
  //       $g('#graphedia_container ul').css('border','none !important');
  //       $g('.comment-container').each(function(index, element){ $g(element).flyOffPage({ 
  //           duration: 800, 
  //           direction: 'random',
  //           tween: {
  //             opacity: 0
  ...
  //         }) 
  //       })
  //       setTimeout("$g('#graphedia_container').empty()", 800);
  //     } else {
  //       _g.setup_socket();
  //       _g.add_markup();
  //     } 
...
Graphedia.prototype.setup_socket = function() {
  _g.socket = io.connect('http://' + URL + '/bmlet');
  _g.join_page();
  ...
  _g.socket.on('comments.new', function(data) {
    var new_comment = new Comment('show', {
      ...
      parent_author_id: data.parent_author_id,
      page_x: data.page_x,
      page_y: data.page_y,
      ups: 0
    })
...
Graphedia.prototype.add_markup = function() {
  ...
  window.addEventListener('message', function(e) {
    ...
    if(e.data._id && e.data.access_token && e.data.username) {
      _g.user = e.data;
...
Graphedia.prototype.confirm_socket = function() {
  _g.socket.emit('users.confirm_socket', _g.user.access_token);
}
...
Graphedia.prototype.display_replies = function(comment) {
  var reply = new Comment('show', {
    ...
    parent_author_id: comment.parent_author_id,
  ...
  if(comment.replies) {
    for(var i=0; i<comment.replies.length; i++) _g.display_replies(comment.replies[i]);
  }
}
...
CommentManager.prototype.display = function(comment, opt_reply) {
  ...
  if (opt_reply) ul.attr('id','reply-initial-list')
...
CommentManager.prototype.replace = function(old_comment, new_comment) {
  ...
  $g('#reply-initial-list').removeAttr('id');
}
...
function Comment(method, params) {
  ...
  this.parent_author_id = params.parent_author_id;
...
Comment.prototype.show = function() {
  ...
  var up_arrow = $g('<div>').addClass('up-arrow').click(function() {
    c.upvote();
  });
  
  c.markup.append(ups, author, comment, reply, up_arrow)
...
Comment.prototype.reply = function() {
  ...
  var new_comment = new Comment('create', {
    ...
    parent_author_id: c.author_id
  })
  _g.comment_manager.display(new_comment, true);
  new_comment.ready();
...
Comment.prototype.create = function() {
  ...
  c.markup = $g('<div>').addClass('new-comment-container').html(c.author);
...
Comment.prototype.ready = function() {
  ...
  form.submit(function() {
    ...
    _g.socket.emit('comments.new', {
      ...
      parent_author_id: c.parent_author_id,
      page_x: c.page_x,
      page_y: c.page_y
    }, function(data) {
      if(data.success) {
...
Comment.prototype.upvote = function() {
  ...
  _g.socket.emit('comments.upvote', {
    ...
    if(data.success) {
      var arrow = _g.container.find('#' + c.comment_id).children('.up-arrow');
      arrow.hide()
