...
Graphedia.prototype.setup_socket = function() {
  ...
  _g.socket.emit('graphedia.init', document.URL, function(comment_streams) {
    _g.add_comment_streams(comment_streams)
  });
  
  _g.socket.on('comments.new', function(data) {
    _g.container.append(
      new Comment('add', { comment_id: data.comment_id, comment: data.comment, author: data.author, parent_id: data.parent_id, x: data.x, y: data.y, ups: 0 })
    );
  })
...
Graphedia.prototype.add_markup = function() {
  ...
  $g(window).click(function(e) {
    if(e.shiftKey && _g.user.access_token) {
      var new_comment = new Comment('create', { author: _g.user, parent_id: 0, x: e.pageX, y: e.pageY })
...
Graphedia.prototype.add_comment_streams = function(comment_streams) {
  for(i in comment_streams) {
    if(comment_streams[i].parent_id == 0) {
      var comments = comment_streams[i].comments;
    }
  }
...
Comment.prototype.add = function() {
  ...
  var comment = $g('<div>').addClass('comment').css({
    ...
    'top': (c.y-10) +'px',
    'left': (c.x-13)+'px'
  }).html(c.comment).attr('id',c.comment_id); //c.author + ' says ' + 
  
  var upvote = $g('<div>').html('upvote').click(function() {
    c.upvote();
  })
  
  var reply = $g('<a>').html('reply').click(function() {
    var new_comment = new Comment('create', { author: _g.user, parent_id: c.comment_id, x: c.x, y: c.y })
    _g.container.append(new_comment.markup);
    new_comment.obj.ready(_g, new_comment.markup, new_comment.comment, new_comment.form);
  })
  ...
  comment.append($g('<br />'),reply,$g('<br />'),ups,upvote)
...
Comment.prototype.create = function() {
  ...
  var comment = $g('<textarea>').addClass('commentField').attr('name','new_comment').css({'font-size': '12px', 'padding': '10px', 'width':'210px'});
...
Comment.prototype.ready = function(g, markup, comment, form) {
  ...
  form.submit(function() {
    ...
    g.socket.emit('comments.new', { access_token: _g.user.access_token, comment: body, parent_id: c.parent_id, x: c.x, y: c.y }, function(data) {
      if(data.success) {
        markup.remove();
        g.container.append(
          new Comment('add', { comment_id: data.comment_id, comment: body, author: _g.user, parent_id: data.parent_id, x: c.x, y: c.y, ups: 0 })
        );
      }
    })
    return false;
