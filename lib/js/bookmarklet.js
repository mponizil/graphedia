...
load_jq.try_ready = function(time_elapsed) {
  if (typeof jQuery == "undefined") {
    if (time_elapsed <= 5000) setTimeout("load_jq.try_ready(" + (time_elapsed + 100) + ")", 100);
...
load_elastic.try_ready = function(time_elapsed) {
  if (typeof abeshit == "undefined") {
    if (time_elapsed <= 5000) setTimeout("load_elastic.try_ready(" + (time_elapsed + 100) + ")", 100);
...
load_color = function() {
  load_js('http://' + URL + '/javascripts/jquery.color.min.js',load_color.try_ready)
}
load_color.try_ready = function(time_elapsed) {
  if (typeof mishit == "undefined") {
    if (time_elapsed <= 5000) setTimeout("load_color.try_ready(" + (time_elapsed + 100) + ")", 100);
    else alert("Timed out while loading jQuery Color.")
  } else {
...
Graphedia.prototype.setup_socket = function() {
  ...
  _g.socket.emit('graphedia.init', document.URL, function(data) {
...
CommentManager.prototype.display = function(comment) {
  ...
  var li = $g('<li>');
  _g.top_z++;
  var cp_div = $g('<div>').addClass('comment-parent').attr('id','cp_' + comment.comment_id).css('z-index',_g.top_z)
  ul.append(li);
  li.append(cp_div)
...
Comment.prototype.show = function() {
  ...
  var reply = $g('<a>').addClass('reply').html('reply').click(function() {
    c.reply();
  })
...
Comment.prototype.create = function() {
  ...
  c.markup = $g('<div>').addClass('new-comment'); //.css({ top: (c.page_y-10), left: (c.page_x-13) }); //i don't know why it starts out 50 pixels below, but whatever.
